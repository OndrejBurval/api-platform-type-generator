...
export default class {
  ...
  async createFile(template, dest, context = {}, warn = true) {
    ...
    if (template.endsWith(".js")) {
      ...
      content = await prettier.format(content, { parser: "babel-ts" });
    }
  ...
  async createEntrypoint(entrypoint, dest) {
    await this.createFile("entrypoint.js", dest, { entrypoint }, false);
  }

  async createConfigFile(dest, context) {
    await this.createFile("utils/config.ts", dest, context, false);
  }
